...
from utils.attack import generationAttack
...
parser.add_argument('--feature_ratio', default=0.1, type=float, help='feature ratio of adversary')
...
# setup logger
filename = "{}_{}_s{}_t{}.txt".format(time.strftime('%Y%m%d%H%M%S'), args.dataset_name, args.num_party, args.source,
                                      args.target)
...
if os.path.exists(os.path.join(modelbase, directory)):
    ...
    generation_asr = generationAttack(num_dim=args.encode_length, target=args.target, source=args.source,
                                      ...
                                      num_classes=num_classes, amount=10, threshold=0.9, workers=workers, server=server,
                                      data_loaders=[train_data_loader, test_data_loader], device=device)
    torch.save(generation_asr,
               os.path.join(resultbase, directory, 'generation_asr_s{}_t{}.pt'.format(args.source, args.target)))
else:
